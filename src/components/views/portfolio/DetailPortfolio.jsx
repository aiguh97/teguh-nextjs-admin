...
const DetailPortfolio = ({ data }) => {
  ...
  console.log(data);
  const imageUrl = getFile(data.thumbnail);
  return (
    <div className="card">
      ...
      <table className="mt-4 mb-1 w-full border text-sm text-left dark:bg-gray-800 text-gray-500 dark:text-gray-400">
        <tbody className="text-xs text-gray-700   dark:text-gray-400 ">
          ...
          <tr className="border-b border-stroke hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td className="px-6 py-4 flex gap-1 items-center ">{data.name}</td>
          </tr>
          <tr className="border-b border-stroke hover:bg-gray-50 dark:hover:bg-gray-600">
            <th scope="col" className="px-6 py-4 whitespace-nowrap">
              Type
          ...
          <tr className="border-b border-stroke hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td className="px-6 py-4">{data.excerpt}</td>
          </tr>
          <tr className="border-b border-stroke hover:bg-gray-50 dark:hover:bg-gray-600">
            <th scope="col" className="px-6 py-4 whitespace-nowrap">
              Category
              ...
              <div className="flex gap-2">
                {data.skill.map((skill, index) => (
                  <div className="flex" key={index}>
                    <div className="badge">{skill.label}</div>
                  </div>
                ))}
              </div>
            </td>
      ...
      <Image src={imageUrl} alt={data.name} width={1200} height={600} />

      <hr className="hr" />
      <MDEditorPreview source={data.content} className="md:p-4 rounded-lg" />
      <PopupModal
